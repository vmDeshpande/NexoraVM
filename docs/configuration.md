...
VM definitions are stored separately in `vm-definitions.json`. A definition contains a `VmConfiguration` plus a separate runtime status. New definitions receive generated UUID-based IDs and start as `stopped`.
